<template>
  <Card shadow>
    <div>
      <div class="message-page-con message-category-con">
        <Menu width="auto" active-name="unread" @on-select="handleSelect">
          <MenuItem name="unread">
            <span class="category-title">消息</span>
            <!-- <Badge id="redcolor" style="margin-left: 10px" :count="messageUnreadCount"></Badge> -->
          </MenuItem>
      ...
      <div class="message-page-con message-list-con" style="overflow: auto; width: 300px;">
        ...
        <Menu width="auto" active-name :class="titleClass" @on-select="handleView">
          <MenuItem v-for="item in messageList" :name="item.msg_id" :key="`msg_${item.msg_id}`">
            <div>
              ...
              <Badge
                status="default"
                :text="item.create_time+ ' ' + (item.sendName ? (item.sendName.length>6 ? item.sendName.substr(0,6)+'...':item.sendName) : '')"
              />
              <Button
              ...
              <Button
                ...
                v-show="currentMessageType !== 'unread'"
              ></Button>
              <!--  @click.native.stop="removeMsgDel(item)" -->
            </div>
          </MenuItem>
        </Menu>
      </div>

      <div class="message-page-con message-view-con" style="left: 520px;">
        <Spin fix v-if="contentLoading" size="large"></Spin>
        <div class="message-view-header">
          <h2 class="message-view-title">{{ showingMsgItem.title }}</h2>
          <time
            class="message-view-time"
          >{{ showingMsgItem.create_time }} {{showingMsgItem.sendName ? (showingMsgItem.sendName.length>6 ? showingMsgItem.sendName.substr(0,6)+'...':showingMsgItem.sendName):''}}</time>
        </div>
...
#redcolor .ivu-badge-count-alone {
  border-color: red;
  color: red;
}
</style>
