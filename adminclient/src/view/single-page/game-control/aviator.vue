<template>
    <div>
     ...
     <Card style="margin-top:10px">
      <div v-if="gameSettingData.auto_play!=null&&gameSettingData.auto_play.length==4">
        <p style="color:red;margin-top:10px;">自动游戏开奖算法（无人投注）:</p>
        <ul id="auto_pay">
          <li><span>下限：{{gameSettingData.auto_play[0].min}}</span><span>上限：{{gameSettingData.auto_play[0].max}}</span><span>命中概率：{{gameSettingData.auto_play[0].odds}}</span></li>
          <li><span>下限：{{gameSettingData.auto_play[1].min}}</span><span>上限：{{gameSettingData.auto_play[1].max}}</span><span>命中概率：{{gameSettingData.auto_play[1].odds}}</span></li>
          <li><span>下限：{{gameSettingData.auto_play[2].min}}</span><span>上限：{{gameSettingData.auto_play[2].max}}</span><span>命中概率：{{gameSettingData.auto_play[2].odds}}</span></li>
          <li><span>下限：{{gameSettingData.auto_play[3].min}}</span><span>上限：{{gameSettingData.auto_play[3].max}}</span><span>命中概率：{{gameSettingData.auto_play[3].odds}}</span></li>
        </ul>
      </div>
      <div style="margin-top:10px;">
        <Button 
                @click="showAutoPlaySettingPage"
                type="primary"
              >
              修改
              </Button>
      ...
      <Modal
          v-model="showAutoPlaySetting"
          title="自动游戏开奖算法（无人投注）"
          @on-ok="saveBairenGameSettingData(false)"
      >
          <div v-if="gameSettingData.auto_play!=null&&gameSettingData.auto_play.length==4">
              <ul id="edit_atuto_pay">
                  <li><div>下限:<Input type="number" v-model.number="gameSettingData.auto_play[0].min"/></div><div>上限:<Input type="number" v-model.number="gameSettingData.auto_play[0].max"/></div><div>概率:<Input type="number" v-model.number="gameSettingData.auto_play[0].odds"/></div></li>
                  <li><div>下限:<Input type="number" v-model.number="gameSettingData.auto_play[1].min"/></div><div>上限:<Input type="number" v-model.number="gameSettingData.auto_play[1].max"/></div><div>概率:<Input type="number" v-model.number="gameSettingData.auto_play[1].odds"/></div></li>
                  <li><div>下限:<Input type="number" v-model.number="gameSettingData.auto_play[2].min"/></div><div>上限:<Input type="number" v-model.number="gameSettingData.auto_play[2].max"/></div><div>概率:<Input type="number" v-model.number="gameSettingData.auto_play[2].odds"/></div></li>
                  <li><div>下限:<Input type="number" v-model.number="gameSettingData.auto_play[3].min"/></div><div>上限:<Input type="number" v-model.number="gameSettingData.auto_play[3].max"/></div><div>概率:<Input type="number" v-model.number="gameSettingData.auto_play[3].odds"/></div></li>
              </ul>
          </div>
      </Modal>
     </Card>
 ...
 export default {
   ...
   methods: {
     ...
     async saveBairenGameSettingData() {
       ...
       let aviatorConfig = {};
       aviatorConfig.award_odds=[];
       aviatorConfig.pay_multiple=[];
       aviatorConfig.auto_play = [];
       aviatorConfig.single_control=[];
       aviatorConfig.limits={};
       this.gameSettingData.award_odds.forEach(element => {
        aviatorConfig.award_odds.push({
       ...
       aviatorConfig.pay_multiple=[];
       aviatorConfig.pay_multiple = this.gameSettingData.pay_multiple;
       aviatorConfig.auto_play = [];
       for (let i=0;i<this.gameSettingData.auto_play.length;i++){
          let res = {};
          res.min =  parseFloat(this.gameSettingData.auto_play[i].min);
          res.max =  parseFloat(this.gameSettingData.auto_play[i].max);
          res.odds = parseFloat(this.gameSettingData.auto_play[i].odds);
          if(!isNaN(res.min)&&!isNaN(res.max)&&!isNaN(res.odds)){
            aviatorConfig.auto_play.push(res);
          }else{
            this.$Message.error("自动游戏开奖算法（无人投注）配置有问题！");
            return;
          }
       }
       aviatorConfig.single_control = [];
       for (let i=0;i<this.gameSettingData.single_control.length;i++){
        ...
        aviatorConfig.single_control.push(tmp);
       }
      ...
      for (let key in this.gameSettingData.limits) {
        if (this.gameSettingData.limits.hasOwnProperty(key)) {
          ...
          aviatorConfig.limits[this.gameSettingData.limits[key].name]=item;
        }
      }
       let Data = {
         token: getToken(),
         gameId:this.paramBairen,
         config:JSON.stringify(aviatorConfig),
       };
       console.log(aviatorConfig.limits);
       //提交保存
 ...
 .payMultiple,.edit_pay_multiple, #auto_pay ,#edit_atuto_pay, #singleControl{
  font-size:12pt;
 }

 .payMultiple li, #auto_pay li,#singleControl li{
  display:inline;
  margin-right:20px;
 ...
 .payMultiple li span, #auto_pay li span,#singleControl li span,#limits li span{
  margin-right:20px;
 }

 #edit_atuto_pay li,.edit_pay_multiple li {
  list-style: none;
  margin-top:5px;
  padding:5px 5px;
 ...
 #edit_atuto_pay li div,.edit_pay_multiple li div {
  margin-left:20px;
  width:80px;
