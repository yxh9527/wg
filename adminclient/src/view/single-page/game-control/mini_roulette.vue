          ...
          >
              <div v-if="currentCurrencyLimitsSettingData">
                  <div>
                      ...
                      <span>游戏内兑换USD汇率</span>
                      <Input type="text" v-model="currentCurrencyLimitsSettingData.exchange" />
                  </div>
              </div>
              <div v-else></div>
          </Modal>
   ...
   methods: {
     ...
     showCurrcenyLimitsPage(row,isAdd) {
      if (!isAdd){
         ...
         this.currentCurrencyLimitsSettingData.exchange = row.exchange;
         ...
         this.currentCurrencyLimitsSettingData.exchange =0;
     ...
     async saveBairenGameSettingData() {
      ...
      for (let key in this.gameSettingData.limits) {
        if (this.gameSettingData.limits.hasOwnProperty(key)) {
          ...
          item.exchange = parseFloat(this.gameSettingData.limits[key].exchange);
          item.bet_list=[];
