 ...
 export default {
   ...
   methods: {
     ...
     async saveBairenGameSettingData() {
       ...
       diceConfig.xtable = this.gameSettingData.xtable;
       diceConfig.fix_hit = this.gameSettingData.fix_hit;
       this.gameSettingData.award_odds.forEach(element => {
        diceConfig.award_odds.push({
      ...
      for (let key in this.gameSettingData.limits) {
        if (this.gameSettingData.limits.hasOwnProperty(key)) {
          ...
          diceConfig.limits[this.gameSettingData.limits[key].name]=item;
        }
      }
       let Data = {
         token: getToken(),
         gameId:this.paramBairen,
         config:JSON.stringify(diceConfig),
       };
       //提交保存
