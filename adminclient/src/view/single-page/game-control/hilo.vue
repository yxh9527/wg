 ...
 export default {
   ...
   methods: {
     ...
     async saveBairenGameSettingData() {
       ...
       hiloConfig.fix_hit = this.gameSettingData.fix_hit;
       hiloConfig.xtable = this.gameSettingData.xtable;
       this.gameSettingData.award_odds.forEach(element => {
        hiloConfig.award_odds.push({
      ...
      for (let key in this.gameSettingData.limits) {
        if (this.gameSettingData.limits.hasOwnProperty(key)) {
          ...
          hiloConfig.limits[this.gameSettingData.limits[key].name]=item;
        }
      }
       let Data = {
         token: getToken(),
         gameId:this.paramBairen,
         config:JSON.stringify(hiloConfig),
       };
       console.log(hiloConfig.limits);
       //提交保存
